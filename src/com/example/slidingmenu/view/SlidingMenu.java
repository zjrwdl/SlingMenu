...
//		addView(bgShade, bgParams);
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			if (mIsBeingDragged) {
				...
				if (canSlideLeft) {
					if (scrollX > -getInitialShowMenuViewWidth())
						// scrollX = 0;
						scrollX = -getInitialShowMenuViewWidth();
				}
				if (canSlideRight) {
					if (scrollX < 0)
						scrollX = 0;
			...
			if (mIsBeingDragged) {
				...
				if (oldScrollX <= 0 && canSlideLeft) {// left view
					if (xVelocity > VELOCITY) {
						...
						dx = -oldScrollX - getInitialShowMenuViewWidth();
						Log.d("JUIDBG","getInitialShowMenuViewWidth()="+getInitialShowMenuViewWidth()+" getMenuViewWidth()/2="+getMenuViewWidth() / 2);
					} else if (oldScrollX < - getInitialShowMenuViewWidth()) {
						dx = -getMenuViewWidth() - oldScrollX;
					} else if (oldScrollX >= (-getMenuViewWidth()+getInitialShowMenuViewWidth()) * 1 / 2) {
						dx = -oldScrollX - getInitialShowMenuViewWidth();
						if (hasClickLeft) {
							hasClickLeft = false;
	...
	private int getInitialShowMenuViewWidth() {
		if (mMenuView == null) {
			return 0;
		}
		return (int) mContext.getResources().getDimension(R.dimen.sliding_menu_leftview_show_width);
	}
	...
	public void showLeftViews() {
		mMenuView.setVisibility(View.VISIBLE);
		mDetailView.setVisibility(View.INVISIBLE);
		smoothScrollTo(-getInitialShowMenuViewWidth());
	}
